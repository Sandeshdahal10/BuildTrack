...
<script>
    (function () {
        var dial = document.getElementById('dialCode');
        var contact = document.getElementById('contact');

        if (!dial || !contact) return;

        // If user already has a +code in the field (e.g., after validation error), auto-select it.
        function tryAutoSelectFromContact() {
            var val = (contact.value || '').trim();
            if (!val.startsWith('+')) return;

            // Match the longest option prefix (+1-xxx etc.)
            var best = '';
            for (var i = 0; i < dial.options.length; i++) {
                var optVal = dial.options[i].value;
                if (!optVal) continue;
                if (val.startsWith(optVal) && optVal.length > best.length) {
                    best = optVal;
                }
            }
            if (best) dial.value = best;
        }
        ...
        // When country code changes, prepend it to the contact input (if not already present).
        dial.addEventListener('change', function () {
            var code = (dial.value || '').trim();
            var val = normalizeSpaces(contact.value);
            if (!code) return;
            if (!val) {
                contact.value = code + ' ';
                contact.focus();
            ...
            // If already has any +code, replace it with selected code.
            if (val.startsWith('+')) {
                val = val.replace(/^\+[^\s]+\s*/, '');
            }
            contact.value = code + ' ' + val;
        });

        tryAutoSelectFromContact();
    })();
</script>
